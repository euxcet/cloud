...
int server_init() {
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_REUSEPORT, (const void *)&on, sizeof(on)) < 0) {
        printf("Error: enable port use failed\n");
        return -1;
    }
...
int tun_init() {
    ...
    strncpy(ifr.ifr_name, "over35", IFNAMSIZ);
    ...
    system("ifconfig over35 " ADDRESS_PREFIX ".1 netmask 255.255.255.0 up");
...
void process_tun(int tun_fd) {
    ...
    // read data from tun
	printf("!!!\n");
    if ((size = read(tun_fd, buf+nread, 10)) < 0) {
...
void process_client(int client_fd, int tun_fd, int epoll_fd) {
    ...
    while(true) {
        ...
        if (msg.type == TYPE_IP_REQUEST) {
			...
			//printf(" client INET_REQUEST  %d    %d %d\n", datalen, msglen, sizeof(message));
...
int main(int argc, char **argv) {
	...
	/*
	pthread_t id;
	pthread_create(&id, NULL, &process_tun, &tun_fd);
